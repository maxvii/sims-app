      ...
      <div className="flex gap-1 px-4 pt-4 overflow-x-auto scrollbar-hide">
        {tabs.map((t) => (
          <button
            ...
            className={`px-4 py-2 rounded-full text-xs font-semibold whitespace-nowrap transition-all ${activeTab === t.id ? 'bg-pink-500 text-white shadow-md' : 'liquid-glass-pill text-gray-500'}`}
          >
            {t.label}
          </button>
        ))}
                ...
                <div className="flex flex-wrap gap-2">
                  {STATUSES.map((s) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${event.status === s ? statusColors[s] + ' ring-2 ring-pink-300' : 'bg-white/50 text-gray-500 hover:bg-white/70'}`}
                    >
                      {s}
                    </button>
                  ))}
          ...
          <div className="space-y-4">
            <form onSubmit={handleComment} className="liquid-glass-card p-4">
              <textarea
                ...
                rows={3}
                className="w-full bg-white/50 rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-pink-200 resize-none border border-white/40"
              />
            ...
            {event.comments?.map((c) => (
              <div key={c.id} className="liquid-glass-card p-4 animate-fade-in">
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-7 h-7 rounded-full bg-gradient-to-br from-pink-400 to-coral flex items-center justify-center">
            ...
            {(session?.user?.role === 'ADMIN' || session?.user?.role === 'EDITOR' || session?.user?.role === 'APPROVER') && (
              <div className="liquid-glass-card p-4">
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full bg-white/50 rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-pink-200 resize-none border border-white/40 mb-3"
                />
          ...
          <div className="space-y-4">
            <form onSubmit={handleAddRef} className="liquid-glass-card p-4 space-y-3">
              ...
              <input
                ...
                className="w-full bg-white/50 rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-pink-200 border border-white/40"
              />
              <input
                value={refUrl}
              ...
              <a key={r.id} href={r.url} target="_blank" rel="noopener noreferrer" className="liquid-glass-card p-4 flex items-center gap-3 hover:scale-[1.01] transition-all block">
                <div className="w-10 h-10 rounded-xl bg-pink-50 flex items-center justify-center flex-shrink-0">
                  {r.type === 'IMAGE' ? (
                    ...
                    <svg className="w-5 h-5 text-pink-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/></svg>
                  )}
                </div>
  ...
  return (
    <div className="liquid-glass-card p-4">
      <div className="flex items-center gap-3 mb-2">
        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-mauve to-pink-400 flex items-center justify-center flex-shrink-0">
