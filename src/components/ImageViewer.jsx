    ...
    <div className="space-y-3">
      ...
      <div className="relative rounded-2xl overflow-hidden">
        ...
        {showPins && pinComments.map((c) => (
          <button
            ...
            className={`absolute w-7 h-7 -translate-x-1/2 -translate-y-1/2 rounded-full flex items-center justify-center shadow-lg transition-all z-10 ${activePin === c.id ? 'bg-pink-500 scale-125' : 'bg-white/90 hover:bg-pink-100'}`}
            style={{ left: `${c.pinX}%`, top: `${c.pinY}%` }}
          >
            <svg className={`w-3.5 h-3.5 ${activePin === c.id ? 'text-white' : 'text-pink-500'}`} fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5a2.5 2.5 0 010-5 2.5 2.5 0 010 5z"/></svg>
          </button>
        ))}
        ...
        {activePin && (() => {
          ...
          return (
            ...
            >
              <p className="text-xs font-semibold text-pink-600">{c.user.name}</p>
        ...
        {pendingPin && (
          <div
            className="absolute w-8 h-8 -translate-x-1/2 -translate-y-1/2 rounded-full bg-pink-500 flex items-center justify-center shadow-lg animate-bounce z-20"
        ...
        {pinMode && !pendingPin && (
          <div className="absolute top-3 left-3 right-3 glass-card px-3 py-2 text-center animate-fade-in">
            <p className="text-xs font-semibold text-pink-600">Click anywhere on the image to drop a pin</p>
          </div>
        )}
      </div>
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`flex-1 py-2.5 rounded-xl font-semibold text-sm transition-all ${pinMode ? 'bg-pink-500 text-white shadow-md' : 'bg-white/50 text-gray-600 hover:bg-white/70'}`}
        >
      ...
      {pendingPin && (
        <form onSubmit={handlePostPin} className="glass-card p-4 animate-slide-up space-y-2">
          <p className="text-xs font-semibold text-pink-600">Pin at ({pendingPin.x.toFixed(0)}%, {pendingPin.y.toFixed(0)}%)</p>
          <input
            value={comment}
            onChange={(e) => setComment(e.target.value)}
            placeholder="Add your comment on this spot..."
            className="w-full bg-white/50 rounded-xl px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-pink-200 border border-white/40"
            autoFocus
          />
          <div className="flex gap-2">
      ...
      {!pinMode && !pendingPin && (
        <form onSubmit={handlePostGeneral} className="glass-card p-3 flex gap-2">
          <input
            ...
            className="flex-1 bg-white/50 rounded-xl px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-pink-200 border border-white/40"
          />
          <button type="submit" disabled={submitting || !comment.trim()} className="gradient-btn px-4 py-2 text-xs disabled:opacity-50">
            Post
          </button>
        ...
        <div className="space-y-2">
          ...
          {pinComments.map((c) => (
            <button
              ...
              className={`glass-card p-3 w-full text-left flex items-start gap-3 transition-all hover:scale-[1.01] ${activePin === c.id ? 'ring-2 ring-pink-300' : ''}`}
            >
              <div className="w-7 h-7 rounded-full bg-pink-100 flex items-center justify-center flex-shrink-0">
                <svg className="w-3.5 h-3.5 text-pink-500" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5a2.5 2.5 0 010-5 2.5 2.5 0 010 5z"/></svg>
              </div>
        ...
        <div className="space-y-2">
          ...
          {generalComments.map((c) => (
            <div key={c.id} className="glass-card p-3 flex items-start gap-3">
              <div className="w-7 h-7 rounded-full bg-gradient-to-br from-pink-400 to-coral flex items-center justify-center flex-shrink-0">
