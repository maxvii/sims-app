          ...
          <div className="absolute top-3 left-3 right-3 animate-fade-in">
            {nearbyComments.map((c) => (
              <div key={c.id} className="glass-card px-3 py-2 mb-1 text-xs">
                <span className="font-semibold text-pink-600">{c.user.name}</span>
      ...
      <div className="glass-card p-3">
        <div className="flex items-center gap-3 mb-2">
          <button
            ...
            className="w-9 h-9 rounded-full bg-gradient-to-br from-pink-400 to-coral flex items-center justify-center flex-shrink-0"
          >
            {playing ? (
          ...
          <div className="flex-1 relative">
            <input
              ...
              style={{ background: `linear-gradient(to right, #FF3D7F ${(currentTime / (duration || 1)) * 100}%, #e5e7eb ${(currentTime / (duration || 1)) * 100}%)` }}
            />
            ...
            {comments.filter((c) => c.timestamp != null).map((c) => (
              <button
                ...
                className="absolute top-1/2 -translate-y-1/2 w-2.5 h-2.5 rounded-full bg-pink-500 border-2 border-white shadow-sm hover:scale-150 transition-transform z-10"
        ...
        {comments
          ...
          .map((c) => (
            <button
              ...
              className={`glass-card p-3 w-full text-left flex items-start gap-3 transition-all hover:scale-[1.01] ${activeComment === c.id ? 'ring-2 ring-pink-300' : ''}`}
            >
              <span className="px-2 py-0.5 rounded-lg bg-pink-100 text-pink-600 text-xs font-mono font-bold flex-shrink-0">
